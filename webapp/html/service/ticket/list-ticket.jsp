...
<section>
    ...
    <main>
        ...
        <div class="search-container">
            <form action="list-ticket" method="get">
        ...
        <table class="table table-bordered table-hover mt-3">
            ...
            <tbody>
            <c:forEach items="${listTicket}" var="e">
                <tr>
                    <td scope="row">${e.ticketId}</td>
                    <td>${e.trip.destination}</td>
                    <td>${e.car.licensePlate}</td>
                    <td>${e.customerName}</td>
                    <td>${e.bookingTime}</td>
                    <td class="process">
                        <a href="update-ticket?id=${e.ticketId}" class="view"><i class="fa-solid fa-pen-to-square"></i>Edit</a>
<%--                        <a href="delete-ticket?id=${e.ticketId}" class="view"><i class="fa-solid fa-trash-can"></i>Delete</a>--%>

                        <a type="button" class="view" data-bs-toggle="modal" data-bs-target="#ticket${e.ticketId}">
                            <i class="fa-solid fa-trash-can"></i>Delete
                        </a>

                        <!-- Modal -->
                        <div class="modal fade" id="ticket${e.ticketId}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <p>Confirm delete Ticket with id = ${e.ticketId}        </p>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                        <a class="mx-2 btn btn-danger text-light" href="delete-ticket?id=${e.ticketId}">
            ...
            <nav aria-label="...">
                <ul class="pagination">
                    <c:if test="${tag > 1}">
                        <li class="page-item">
                            <a class="page-link"
                               href="list-ticket?page=${tag - 1}&txtSearch=${txtSearch}&option=${option}">Previous</a>
                        </li>
                    </c:if>
                    <c:forEach begin="1" end="${endP}" var="i">
                        <li class="page-item ${tag == i?"active":""}">
                            <a class="page-link"
                               href="list-ticket?page=${i}&txtSearch=${txtSearch}&option=${option}">${i}</a>
                        </li>
                    </c:forEach>
                    <c:if test="${tag < endP}">
                        <li class="page-item">
                            <a class="page-link"
                               href="list-ticket?page=${tag + 1}&txtSearch=${txtSearch}&option=${option}">Next</a>
                        </li>
                    </c:if>
                </ul>
            </nav>
